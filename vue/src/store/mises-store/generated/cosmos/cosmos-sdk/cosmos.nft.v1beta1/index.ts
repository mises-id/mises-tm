...
export default {
	...
	getters: {
			...
			return state.Supply[JSON.stringify(params)] ?? {}
				...
				getNFTsOfClass: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.NFTsOfClass[JSON.stringify(params)] ?? {}
		},
	...
	actions: {
		...
		async QueryNFTsOfClass({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryNFTsOfClass( key.class_id, query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryNFTsOfClass( key.class_id, {...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
				commit('QUERY', { query: 'NFTsOfClass', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryNFTsOfClass', payload: { options: { all }, params: {...key},query }})
				return getters['getNFTsOfClass']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryNFTsOfClass API Node Unavailable. Could not perform query: ' + e.message)
