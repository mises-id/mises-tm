...
DOCKER_TAG = unstable
DOCKER_IMAGE = mises/mises-tm
MISES_DAEMON_BINARY = misestmd
MISES_CLI_BINARY = misestmcli
...
ifeq ($(OS), Windows_NT)
	go build -mod=readonly $(BUILD_FLAGS) -o build/$(MISES_DAEMON_BINARY).exe ./cmd/$(MISES_DAEMON_BINARY)
# go build -mod=readonly $(BUILD_FLAGS) -o build/$(MISES_CLI_BINARY).exe ./cmd/$(MISES_CLI_BINARY)
else
	go build -mod=readonly $(BUILD_FLAGS) -o build/$(MISES_DAEMON_BINARY) ./cmd/$(MISES_DAEMON_BINARY)
# go build -mod=readonly $(BUILD_FLAGS) -o build/$(MISES_CLI_BINARY) ./cmd/$(MISES_CLI_BINARY)
endif

build-misestm: go.sum
	mkdir -p $(BUILDDIR)
	go build -mod=readonly $(BUILD_FLAGS) -o $(BUILDDIR) ./cmd/$(MISES_DAEMON_BINARY)
...
build-misestm-darwin: go.sum
	GOOS=darwin GOARCH=arm64 CGO_ENABLED=1 $(MAKE) build-misestm
...
install:
	${GO_MOD} go install $(BUILD_FLAGS) ./cmd/$(MISES_DAEMON_BINARY)
# ${GO_MOD} go install $(BUILD_FLAGS) ./cmd/$(MISES_CLI_BINARY)
...
docker-build:
	...
	docker rm misestm || true
	# create a new container from the latest image
	docker create --name misestm -t -i mises/mises-tm:latest misestm
	# move the binaries to the ./build directory
	mkdir -p ./build/
	docker cp misestm:/usr/bin/misestmd ./build/ ; \
	docker cp misestm:/usr/bin/misestmcli ./build/

docker-localnet:
	docker build -f ./networks/local/misestmnode/Dockerfile . -t misestmd/node
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(MISES_DAEMON_BINARY)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(MISES_DAEMON_BINARY)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(MISES_DAEMON_BINARY)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Jobs=4 -SimAppPkg=$(SIMAPP) -ExitOnFail -Genesis=${HOME}/.$(MISES_DAEMON_BINARY)/config/genesis.json 400 5 TestFullAppSimulation
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -name '*.pb.go' | xargs goimports -w -local github.com/mises-id/mises-tm
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/mises-id/mises-tm"
	godoc -http=:6060
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(MISES_DAEMON_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\misestm\Z misestmd/node "misestmd testnet --v 4 -o /misestm --starting-ip-address 192.168.10.2 --keyring-backend=test"
	docker-compose up -d
else
	mkdir -p ./build/
	@$(MAKE) docker-localnet

	if ! [ -f build/node0/$(MISES_DAEMON_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/misestm:Z misestmd/node "misestmd testnet --v 4 -o /misestm --starting-ip-address 192.168.10.2 --keyring-backend=test"; fi
	docker-compose up -d
endif
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)/build\misestm\Z misestmd/node "misestmd unsafe-reset-all --home=/misestm/node0/misestmd"
	@docker run --rm -v $(CURDIR)/build\misestm\Z misestmd/node "misestmd unsafe-reset-all --home=/misestm/node1/misestmd"
	@docker run --rm -v $(CURDIR)/build\misestm\Z misestmd/node "misestmd unsafe-reset-all --home=/misestm/node2/misestmd"
	@docker run --rm -v $(CURDIR)/build\misestm\Z misestmd/node "misestmd unsafe-reset-all --home=/misestm/node3/misestmd"
else
	@docker run --rm -v $(CURDIR)/build:/misestm:Z misestmd/node "misestmd unsafe-reset-all --home=/misestm/node0/misestmd"
	@docker run --rm -v $(CURDIR)/build:/misestm:Z misestmd/node "misestmd unsafe-reset-all --home=/misestm/node1/misestmd"
	@docker run --rm -v $(CURDIR)/build:/misestm:Z misestmd/node "misestmd unsafe-reset-all --home=/misestm/node2/misestmd"
...
.PHONY: build-docker-local-misestm localnet-start localnet-stop